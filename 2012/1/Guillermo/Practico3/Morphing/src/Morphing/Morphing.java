...
public class Morphing extends Observable {
    ...
    public BufferedImage morphingImagen(BufferedImage imageInitial, BufferedImage imageFinal, int k) {
        int ini = 0;
        int img = 0;
        int fin = 0;
        ...
        for (int i = 0; i < imageInitial.getWidth(); i++) {
            for (int j = 0; j < imageFinal.getHeight(); j++) {
                ini = imageInitial.getRGB(i, j);
                fin = imageFinal.getRGB(i, j);
                img = ini + ((fin - ini) /(pasos))*k;
                imagenActual.setRGB(i, j, img);
                setChanged();
                notifyObservers();
            }
        }
